...
class ColegioListTest(TestCase):
    ...
    def test_list_ok(self):
        resp = self.client.get('/api/v1/colegios/')
    ...
    def test_retrieve_ok(self):
        resp = self.client.get(f'/api/v1/colegios/{self.c1.id}/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data['nombre'], 'Colegio Alfa')

    def test_filtro_ciudad(self):
        resp = self.client.get('/api/v1/colegios/?ciudad=Medellín')
        self.assertEqual(resp.data['count'], 1)

    def test_search_nombre(self):
        resp = self.client.get('/api/v1/colegios/?search=Alfa')
        self.assertEqual(resp.data['count'], 1)

    def test_post_no_permitido(self):
        resp = self.client.post('/api/v1/colegios/', {'nombre': 'X'}, format='json')
...
class ColegioAnioListTest(TestCase):
    ...
    def test_list_ok(self):
        resp = self.client.get('/api/v1/colegios-anio/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data['count'], 2)
    ...
    def test_nombre_viene_de_colegio(self):
        resp = self.client.get(f'/api/v1/colegios-anio/{self.ca1.id}/')
        self.assertEqual(resp.data['nombre'], 'Col X')

    def test_filtro_anio(self):
        resp = self.client.get('/api/v1/colegios-anio/?anio=2025')
        self.assertEqual(resp.data['count'], 1)
    ...
    def test_filtro_activo(self):
        ...
        resp = self.client.get('/api/v1/colegios-anio/?activo=true')
        self.assertEqual(resp.data['count'], 2)
