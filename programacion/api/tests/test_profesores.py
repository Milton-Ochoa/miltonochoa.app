...
class ProfesorListTest(TestCase):
    ...
    def test_nombre_corto_calculado(self):
        resp = self.client.get(f'/programacion/api/v1/profesores/{self.p2.id}/')
    ...
    def test_nombre_corto_sin_apellido(self):
        ...
        resp = self.client.get(f'/programacion/api/v1/profesores/{p.id}/')
        self.assertEqual(resp.data['nombre_corto'], 'Carlos')

    def test_retrieve_ok(self):
        resp = self.client.get(f'/programacion/api/v1/profesores/{self.p1.id}/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data['nombre'], 'Ana')

    def test_retrieve_no_existe_404(self):
        resp = self.client.get('/programacion/api/v1/profesores/99999/')
        self.assertEqual(resp.status_code, 404)

    def test_filtro_ciudad(self):
        resp = self.client.get('/programacion/api/v1/profesores/?ciudad=Medellín')
        self.assertEqual(resp.data['count'], 1)
        self.assertEqual(resp.data['results'][0]['nombre'], 'Juan')

    def test_filtro_departamento(self):
        resp = self.client.get('/programacion/api/v1/profesores/?departamento=Cundinamarca')
        self.assertEqual(resp.data['count'], 1)

    def test_search_nombre(self):
        resp = self.client.get('/programacion/api/v1/profesores/?search=Ana')
        self.assertEqual(resp.data['count'], 1)

    def test_search_documento(self):
        resp = self.client.get('/programacion/api/v1/profesores/?search=123456')
        self.assertEqual(resp.data['count'], 1)
        self.assertEqual(resp.data['results'][0]['nombre'], 'Juan')
    ...
    def test_post_no_permitido(self):
        resp = self.client.post('/programacion/api/v1/profesores/', {'nombre': 'X'}, format='json')
