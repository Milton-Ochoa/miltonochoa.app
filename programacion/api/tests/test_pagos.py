...
class PagoListTest(TestCase):
    ...
    def test_list_ok(self):
        resp = self.client.get('/programacion/api/v1/pagos/')
    ...
    def test_retrieve_ok(self):
        resp = self.client.get(f'/programacion/api/v1/pagos/{self.pago.id}/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data['valor'], 80000)

    def test_filtro_profesor(self):
        resp = self.client.get(f'/programacion/api/v1/pagos/?profesor={self.prof.id}')
        self.assertEqual(resp.data['count'], 1)

    def test_filtro_colegio(self):
        resp = self.client.get(f'/programacion/api/v1/pagos/?colegio={self.ca.id}')
        self.assertEqual(resp.data['count'], 1)

    def test_filtro_desde_hasta(self):
        resp = self.client.get('/programacion/api/v1/pagos/?desde=2026-04-01&hasta=2026-04-30')
        self.assertEqual(resp.data['count'], 1)

    def test_filtro_hasta_excluye(self):
        resp = self.client.get('/programacion/api/v1/pagos/?hasta=2026-03-31')
        self.assertEqual(resp.data['count'], 0)

    def test_create_pago_ok(self):
        prof2 = Profesor.objects.create(nombre='Clara', apellido='Ríos')
        resp = self.client.post('/programacion/api/v1/pagos/', {
            'profesor': prof2.id,
            'colegio': self.ca.id,
            'fecha': '2026-04-10',
            'horas': 1.5,
    ...
    def test_create_pago_campos_faltantes_400(self):
        resp = self.client.post('/programacion/api/v1/pagos/', {'profesor': self.prof.id}, format='json')
        self.assertEqual(resp.status_code, 400)

    def test_delete_no_permitido(self):
        resp = self.client.delete(f'/programacion/api/v1/pagos/{self.pago.id}/')
