...
- [x] Fase 5 — Enrutado por áreas + login único. `core/urls.py` ahora es router AAMO: `/`→`seleccion_area`, `usuarios/` GLOBAL (login en `/usuarios/login/`), `programacion/`→`programacion/urls.py` (agrupa el área; `home`=kanban en `/programacion/`). Decisiones del dueño: login global en `/usuarios/` y API bajo `/programacion/api/v1/`. `seleccion_area`+`_areas_del_usuario` en core/views.py (acceso vía grupo `area:programacion` o perfil colegio/profesor; superuser=todas). Plantillas `core/seleccion_area.html` y `core/sin_area.html`. Middleware actualizado a prefijos `/programacion/` y login global. `LOGIN_URL=/usuarios/login/`. `login_redirect` (superuser/fallback)→`seleccion_area`. Paths hardcodeados corregidos en 8 templates/JS y 142 reemplazos en tests (anclados a comilla/`=` y ordenados usuarios→configuracion para evitar dobles prefijos). Placeholders `logistica/` y `financiera/` (`__init__.py`+README). **Verificación:** `check` limpio, `makemigrations --check`: No changes, **167/167 tests OK** (= baseline), humo: `/`→302→`/programacion/`, kanban/colegios/api responden 200.
- [x] Fase 6 — Importador del backup Excel. Command `programacion/configuracion/management/commands/importar_backup.py`, invocable `python manage.py importar_backup backups/AAMO_export.xlsx`. Inspeccionadas las 17 hojas (valores llegan como str ISO para fechas/horas, int para FK `_id`, bool nativo, None para vacío). Diseño: **PKs preservadas** (`update_or_create(id=...)`, FK por id encajan sin resolver por nombre) → **idempotente** (re-ejecución = actualizados, verificado); orden por dependencias; `_coerce_none` para char/text no-nullable (User.first_name/email); campos `auto_now_add`/`auto_now` (`Tarea.fecha_creacion`, `AlertaAuditoria.detectado`, `HistorialCambio.fecha`, `Informe.creado_en/actualizado_en`) reescritos vía `QuerySet.update()` tras crear (conserva timestamps reales); todo en `transaction.atomic`. **Correcciones al mapeo del PLAN:** `Grados` e `HistorialCambios` viven en app `colegios` (no configuracion/auditoria). **Limitaciones del backup** (reportadas al terminar): contraseñas no vienen → `set_unusable_password` (usar superuser de prueba/reset); M2M (`Profesor.materias`, `AlertaAuditoria.colegios_implicados`) sin hoja → no se reconstruyen. **Verificación:** BD SQLite limpia + `migrate` + import → conteos exactos vs Excel (Materias 9, Grados 9, Libros 11, Colegios 28, Profesores 81, Usuarios 3, ColegioAnio 28, Unidades 183, Bloques 120, Asignaciones 64, ClasesParticulares 4, Informes 0, Tareas 8, Alertas 193, Historial 1557, UsuarioColegio 1, UsuarioProfesor 1 = 2300 filas). FK íntegras (Bloque→ColegioAnio→Colegio→Grado OK), timestamps preservados al microsegundo, encoding UTF-8 correcto (Cúcuta=`0xfa`), `check` y `makemigrations --check` limpios. BD previa (Fase 5) resguardada en `db.sqlite3.fase5.bak`.
- [x] Fase 7 — Verificación final. **Suite completa: 167/167 tests OK** (idéntico al baseline; 100s). **Tablas: 30, set idéntico al baseline** (`baseline_tablas.txt` lista 30 nombres reales + 1 línea de ruido; diff = 0 tablas distintas) y `migrate --check` exit 0 (sin migraciones pendientes). `check` y `makemigrations --check` limpios. **Smoke automatizado con datos cargados** (test client + superuser temporal, host `testserver`): `/`→302→`/programacion/`, kanban/colegios/profesores/informes/auditoría = 200, `/programacion/profesores/` renderiza datos reales ("Gabriela"), API `/programacion/api/v1/colegios/` = 200 (dict paginado DRF). `/programacion/pendientes/` da 404 **por diseño** (esa app solo expone acciones cambiar-estado/editar-tarea; el tablero es el home del área). **Docs actualizadas:** README.md (estructura AAMO por áreas, nota de labels, roles con rutas `/programacion/...`, login único+seleccion_area, prefijo API `/programacion/api/v1/`, instalación con SQLite local vía `.env.dev-api` + paso `importar_backup`, default `BACKUP_DIR`) y **creado CLAUDE.md** (guía interna: estructura, convención ruta≠label, enrutado/login, cómo correr, importador, deuda técnica). **Pendiente para el dueño:** humo visual en navegador (login real con su superusuario, navegación y descarga de un Excel de horario) — las contraseñas del backup son inutilizables, usar superusuario creado con `createsuperuser`.
